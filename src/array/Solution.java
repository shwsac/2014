...
public class Solution {
	...
	public static int minCost(int[][] cost, int m, int n) {
		...
		int totalCostRows = cost.length - m;
		int totalCostCols = cost[0].length - n;
		int totalCost[][] = new int[totalCostRows][totalCostCols];

		totalCost[0][0] = cost[0 + m][0 + n];

		for (i = 1; i < totalCostRows; i++)
			totalCost[i][0] = totalCost[i - 1][0] + cost[i + m][n];

		for (j = 1; j < totalCostCols; j++)
			totalCost[0][j] = totalCost[0][j - 1] + cost[m][j + n];

		for (i = 1; i < totalCostRows; i++)
			for (j = 1; j < totalCostCols; j++)
				totalCost[i][j] = Math.min(totalCost[i - 1][j],
						totalCost[i][j - 1]) + cost[i + m][j + n];
